...
//converts color object to css rgba color string
colorString = (rgb, a) => {
    return 'rgba(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + ', ' + a + ')';
}

//calculates an approximate luminance value from rgb vals
getLum = (r, g, b) => {
...
mainsPattern = (w, h) => {
    ...
    for (let row = 0; row < (h + 2 * st_spacing) / st_spacing; row++) {
        for (let col = 0; col < (w + 2 * st_spacing) / st_spacing; col++) {
            ...
            let isDiagonal = Math.random() < 0.15; //15% chance of diagonal street connecting two grid streets
            if (isDiagonal) {
